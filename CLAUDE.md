...
3. **Class storage = one `course_runs` row.** Columns: `product_id`, `course_sku`, `course_start_date`, `course_start_time`, `course_end_date`, `course_end_time`, `trainer_option_id`, `vacancy`. PK `run_id`. Human label `class_id` formatted `C######` (e.g. `C000042`) — a single per-site sequence starting `C000001`, uniform on every site (2026-07 change; migration 832 renumbered the historical `SG######`/`MY######`/`GH######`/`NG######` ids). Generated by `MMD_RoleManager_Helper_Data::nextClassId()` with `CLASS_ID_PREFIX` — reuse this helper, do not invent another ID scheme.
...
- `app/code/local/MMD/RoleManager/Model/CourseRunEnrolmentService.php` — parses date/time custom options, finds or creates `course_runs`, inserts into `course_run_enrolments`. 20+ date-format fallbacks. Idempotent via `INSERT IGNORE`. Only non-WSQ / unfunded C-prefix course codes (`C` + digit) materialise classes — TGS- (external system), M- (partner legacy) and all other SKUs are skipped, as are bundle/configurable children (guard mirrored in the admin Create New Class action and the Agent API `add_class` op; migration 846 removed the historical non-C runs).
...
- `app/code/local/MMD/RoleManager/Helper/Data.php::nextClassId()` — generates the `C000042`-style identifiers (uniform `C` prefix via `CLASS_ID_PREFIX`).
...
- Don't rename `class_id` or alter the `C######` format — downstream UI deep-links and learner-facing certificates depend on it. (Historical cert numbers like `SG000042-001` printed on already-issued certificates keep their old prefix — never rewrite `cert_no`.)
...
| **CourseImage** | AI cover-image renderer + funding-badge tags. The cover dialog's badge checkboxes drive both the rendered PNG **and** Magento tag writes (`syncProductTags`), so the storefront chips and the cover are guaranteed to match. Storefront catalog list / product view read `getProductBadges()` (filtered to the 9 canonical names) and render colored pills under the course title. Canonical vocabulary: `WSQ, SkillsFuture Credit, PSEA, UTAP, IBF, HRDF, SFEC, Absentee Payroll, MCES` — defined in `Helper/Data.php::getAllBadges()`. CSS palette in `skin/frontend/ultimo/default/css/custom.css` keyed off `getBadgeCssClass()`. Adding a new badge means: append to `getAllBadges()`, add CSS class, seed a `tag` row. **The tags ALSO drive the WSQ Funding card on TGS- courses (2026-08)**: `getWsqFundingNarrativeHtml()` renders canonical per-scheme copy (SFEC/SFC/UTAP/PSEA, links generated from the course's own SKU) gated by the badges, and Edit Course's "Funding and Grant" section is six checkboxes writing those tags — the per-course `funding_and_grant` cms/block is retired for TGS- (non-TGS and partner courses still use the block). See memory `project_funding_checkboxes_via_badge_tags`. |
