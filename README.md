...
| 📣 **Autonomous newsletter** | SG-only agentic-flyer pipeline: designs a course flyer Mon & Thu 10am → manager approval (either manager, via email **or** admin) → MailerLite blast Mon & Thu 8am with a static R2-hosted registration QR. Hard cap **2 flyers/week** for the autonomous flow; nothing sends without approval. The admin queue carries a per-row **design brief** — free-text special instructions (steers the AI copy **and** the colour scheme: named schemes or any hex repaint the whole design) plus a pinned intake date validated against the course's real published Course Dates. |
