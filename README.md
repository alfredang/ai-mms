...
| 💰 **Funding & subsidy hooks** | SG SkillsFuture Credit / WSQ / IBF, MY HRDC — funding tiers (Baseline, MCES) auto-calculated. On WSQ courses the funding card is **checkbox-driven**: six Edit Course checkboxes (WSQ/CASL, MCES/SME, SFEC, UTAP, PSEA, Absentee Payroll) write the funding badge tags, and the card renders canonical per-scheme copy with SkillsFuture links generated from the course's own code. Every WSQ course page shows its SSG **Funding Validity** window (seeded from the TPG master list, editable per course in the admin General tab). The funding card's scheme links are proper buttons (SFC / SFEC / UTAP / PSEA), and learners submit their filled PSEA withdrawal form via a dedicated `/psea-submission/` page that emails the attached form straight to admissions (no DB record). Beyond the government schemes, courses can also carry a **Free AI Subscription** perk badge — one tag drives both the storefront pill under the course title and a promotional "6 Months Free AI Subscription" card rendered directly above the WSQ Funding card, so the two can never drift apart. SEO guide pages walk learners through each journey — How to Register a WSQ Course, How to Register an IBF Course (both linked from the Enquiries mega-menu), How to Claim SkillsFuture Credit and How to Download SWDA SOA — each with step-by-step cards, FAQs and HowTo/FAQPage structured data. |
| 📋 **Course-page section cards** | Six per-course CMS block sections — Learning Outcomes, Brochure, Skills Framework, Certification, About IBF Certification, Funding — render as styled cards on every course page (block-first, with regex fallback from the description) and are edited per course in the Course Details tab. Partner-accreditation (ATC) cards — Microsoft Learning Partner, CompTIA, Linux Foundation, Autodesk ATC, Pearson VUE, Kryterion, PSI — render in the left column after the Certification card, followed by the Certification Exam venue card, driven by multi-select Edit Course checkboxes (`atc_partners` attribute) with canonical per-partner copy. |
...
| 👥 **Six-role admin** | Learner / Trainer / Developer / Marketing / Admin / Training Provider with session-based role switching and **per-role page enforcement** — each role reaches only its own pages (learners are allow-listed; trainer/marketing/developer are confined by a controller map; admin & Training Provider see everything). |
| 🎒 **LMS login** | Dedicated `/lmslogin` page on every site for the LMS roles (learner, trainer, developer, marketing) — email + password or **email OTP**; multi-role users pick a role, single-role users land straight on their dashboard. `/learnerlogin` remains as a legacy alias; admin & Training Provider sign in at the staff portal, which accepts every role. |
...
| 📣 **Autonomous newsletter** | SG-only agentic-flyer pipeline: designs a course flyer Mon & Thu 10am → manager approval (either manager, via email **or** admin) → MailerLite blast Mon & Thu 8am with a static R2-hosted registration QR. Hard cap **2 flyers/week** for the autonomous flow; nothing sends without approval. The admin queue carries a per-row **design brief** — free-text special instructions (steers the AI copy **and** the colour scheme: named schemes or any hex repaint the whole design) plus a pinned intake date validated against the course's real published Course Dates; the brief dialog opens automatically the moment a course is added to the queue, so instructions are set before the design run can pick it up. A brief is never silently dropped: if the AI copy step fails, the run aborts instead of shipping the previous design's copy, and the queue row (the only copy of the brief) is restored to the head of the queue for a retry. A colour ask is persisted independently of the copy step, so it repaints even if the wording regeneration fails. |
...
| ✍️ **Agentic lead-magnet blog** | `/blog` with slug URLs, SEO meta + BlogPosting / FAQPage / BreadcrumbList JSON-LD (FAQ schema auto-extracted from each post's native `<details>` accordions), Magento-tag reuse, likes, social share, R2 hero images. An agent team writes 2 posts/week: a research agent (Claude web search) scouts the latest AI topics, a writer agent produces an in-depth post with a branded, topic-themed auto-generated hero (13 keyword-picked themes incl. a Microsoft Copilot family, so a series of posts renders as one set), managers approve via email or the admin timeline, and approved posts publish Tue & Fri 9am, then auto-share to LinkedIn + Facebook. Admins queue the next courses from the Blog pipeline panel. |
...
| 🔍 **AI-Overview-ready schema** | Course pages emit rich Course JSON-LD — SKU as `courseCode`, aggregate review rating, and **real upcoming class dates** as `CourseInstance` entries parsed from the Course Date options — plus BreadcrumbList; the blog emits BlogPosting / FAQPage / CollectionPage. Structured data only, zero visual change. |
...
| **CourseImage** | AI cover-image renderer + funding-badge tags (the same tags drive the storefront chips, the cover, the WSQ Funding card, and the Free AI Subscription perk card). |
...
| **Reschedule** | Learner-facing class reschedule request form (`/class-reschedule.html`) — email OTP verification, enrolled-course picker (MMS `course_runs` + LMS lookup) and a manual fallback for learners with no matching enrolment. Saves a reschedule lead, notifies staff, and pushes WSQ requests to the LMS (non-fatal; falls back to `pending_push`). |
| **MagentoCaptcha** | Cloudflare Turnstile spam protection shared by the storefront lead/reschedule forms. |
| **Courses / Leads** | Course CRUD + admin grid; read-only catalog/schedule/search JSON APIs (`api_wsq`, `api_nonwsq`, `api_courses`, `api_schedule`, `api_search`, `api_faq`, `api_contact`) sharing one auth + envelope via `Helper/Catalogfeed.php`; contact-form lead capture with auto-reply, AI-drafted replies for every lead type — General Enquiry, Corporate Training, Franchise Enquiry and Customised Training (Claude drafts each reply with a type-specific brief, reviewers approve via a confirm-before-send link, approval auto-sends; all replies sign off with a unified support signature with clickable email/WhatsApp contacts) and MailerLite auto-subscribe on capture (opt-outs never re-added), plus a "Send to MailerLite" mass-action and per-lead sync checkbox in the Leads grid. |
...
| **Blog** | CMS-style lead-magnet blog — Marketing → Blog admin with the agentic pipeline timeline (research agent → writer + auto hero → approval → Tue/Fri 9am slots → LinkedIn/Facebook share), a recent-posts table (title, linked course codes, stage, schedule, likes, preview/approve actions), a drag-to-reorder next-course queue, and a rich in-admin article preview modal (desktop/mobile viewports, works for unpublished posts); `/blog/<slug>` storefront with likes + share. |
