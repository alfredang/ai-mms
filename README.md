...
| ✍️ **Agentic lead-magnet blog** | `/blog` with slug URLs, SEO meta + BlogPosting / FAQPage / BreadcrumbList JSON-LD (FAQ schema auto-extracted from each post's native `<details>` accordions), Magento-tag reuse, likes, social share, R2 hero images. An agent team writes 2 posts/week: a research agent (Claude web search) scouts the latest AI topics, a writer agent produces an in-depth post with a branded auto-generated hero, managers approve via email or the admin timeline, and approved posts publish Tue & Fri 9am, then auto-share to LinkedIn + Facebook. Admins queue the next courses from the Blog pipeline panel. |
