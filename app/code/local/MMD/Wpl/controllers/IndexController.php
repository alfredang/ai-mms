<?php
/**
 * Custom Course Development enquiry handler — wpl/index/post.
 *
 ...
 * configured). On success the lead is stored in mmd_wpl_lead and a
 * notification is emailed to the sales mailbox.
 */
class MMD_Wpl_IndexController extends Mage_Core_Controller_Front_Action
{
    const RETURN_URL = 'wpl-development.html';
    ...
    public function postAction()
    {
        ...
        try {
            ...
            $lead = Mage::getModel('mmd_wpl/lead')
                ...
                ->setMessage($comment)
                ->setSource((string) ($post['source'] ?? 'wpl-development'))
    ...
    protected function _notify(MMD_Wpl_Model_Lead $lead)
    {
        Mage::helper('mmd_leadmail')->notify('WPL Development Enquiry', $lead->getName(), $lead->getEmail(), $lead->getTelephone(), array(
            array('Area of Interest / Sector', $lead->getExpertise()),
