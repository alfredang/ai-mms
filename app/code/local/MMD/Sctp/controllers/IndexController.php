<?php
/**
 * Custom Course Development enquiry handler — sctp/index/post.
 *
 ...
 * configured). On success the lead is stored in mmd_sctp_lead and a
 * notification is emailed to the sales mailbox.
 */
class MMD_Sctp_IndexController extends Mage_Core_Controller_Front_Action
{
    const RETURN_URL = 'sctp-program-development.html';
    ...
    public function postAction()
    {
        ...
        try {
            ...
            $lead = Mage::getModel('mmd_sctp/lead')
                ...
                ->setMessage($comment)
                ->setSource((string) ($post['source'] ?? 'sctp-program-development'))
    ...
    protected function _notify(MMD_Sctp_Model_Lead $lead)
    {
        Mage::helper('mmd_leadmail')->notify('SCTP Program Development Enquiry', $lead->getName(), $lead->getEmail(), $lead->getTelephone(), array(
            array('Programme / Sector of Interest', $lead->getExpertise()),
