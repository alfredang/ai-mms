 ...
 *  To: sales@ + angss@; CC: angch@ + tansc@ (config default). */
class MMD_Appeal_IndexController extends Mage_Core_Controller_Front_Action
{
    ...
    public function postAction()
    {
        ...
        try {
            ...
            $startDate  = trim((string) ($post['course_start_date'] ?? ''));
            $endDate    = trim((string) ($post['course_end_date'] ?? ''));
            $assessDate = trim((string) ($post['assessment_date'] ?? ''));
            ...
            if (!Zend_Validate::is($name, 'NotEmpty') || !Zend_Validate::is($email, 'EmailAddress')
                ...
                || !Zend_Validate::is($startDate, 'NotEmpty') || !Zend_Validate::is($endDate, 'NotEmpty')
                || !Zend_Validate::is($assessDate, 'NotEmpty') || !Zend_Validate::is($comment, 'NotEmpty')) {
            ...
            Mage::getModel('mmd_appeal/lead')->setStoreId(Mage::app()->getStore()->getId())->setStoreCode(Mage::app()->getStore()->getCode())
                ...
                ->setCourse($course)->setCourseCode($courseCode)->setCourseStartDate($startDate)->setCourseEndDate($endDate)
                ->setAssessmentDate($assessDate)->setMessage($comment)->setSource('assessment-appeal')->setIp($turnstile->getRemoteIp())
                ->setUserAgent(substr((string) ($_SERVER['HTTP_USER_AGENT'] ?? ''), 0, 255))->setStatus('new')->save();
            Mage::helper('mmd_leadmail')->notify('Assessment Appeal', $name, $email, $telephone, array(
                array('NRIC', $nric),
                array('Course Title', $course),
                array('Course Code', $courseCode),
                array('Course Start Date', $startDate),
                array('Course End Date', $endDate),
                array('Assessment Date', $assessDate),
            ), $comment, explode(',', self::EXTRA_TO));
