 ...
 * @author     MMD Dev Team
 */
class MMD_CustomOptions_Model_Observer {
...
//                }                
    ...
    public function insertNewOrderItem($sku, $optionType, $orderItem, $optionModel, $productOptions, $store, $optionId, $optionTypeId, $connection, $tablePrefix, $reduce, $orderTotalQtyOrdered, $customoptionsIsOnetime, $finalProductPrice, $productTaxClassId) {
        ...
        if ($optionModel instanceof Mage_Catalog_Model_Product_Option) {
            list($price, $priceType, $isProductPrice, $taxClassId, $oldPrice) = $helper->getOptionPriceAndPriceType($optionModel->getPrice(), $optionModel->getPriceType(), $sku, $store);
        } else {
            list($price, $priceType, $isProductPrice, $taxClassId, $oldPrice) = $helper->getOptionTierPriceAndType($optionModel, $optionTotalQty, $store);
        }
        if (!$isProductPrice) $taxClassId = $productTaxClassId;
        
        // get basePrice
        $basePrice = (($priceType=='percent' && $price) ? $finalProductPrice * $price / 100 : $price);         
        if ($isProductPrice && $helper->isSpecialPriceEnabled() && $oldPrice) {
            $basePrice = $oldPrice;
        }
                    ...
                    switch ($productOption->getType()) {
                        ...
                        default:
                            ...
                            if ($skuPolicy==2 || $skuPolicy==3) { // Independent, Grouped
                                ...
                                if (!is_object($result)) continue 2;
