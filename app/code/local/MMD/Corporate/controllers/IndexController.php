...
class MMD_Corporate_IndexController extends Mage_Core_Controller_Front_Action
{
    ...
    public function postAction()
    {
        ...
        try {
            ...
            $session->addSuccess($this->__('Thank you! Your corporate training enquiry has been received — our team will be in touch shortly.'));
    ...
    {
        try {
            $comment = "CORPORATE ON-SITE TRAINING ENQUIRY\n"
            ...
            Mage::getModel('core/email_template')->setDesignConfig(array('area' => 'frontend'))->setReplyTo($email)
                ->sendTransactional(Mage::getStoreConfig('contacts/email/email_template'), Mage::getStoreConfig('contacts/email/sender_email_identity'),
                    Mage::helper('mmd_corporate')->getRecipients(), null, array('data' => $data));
