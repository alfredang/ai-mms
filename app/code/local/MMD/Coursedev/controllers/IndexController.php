<?php
/**
 * Custom Course Development enquiry handler — coursedev/index/post.
 *
 ...
 * configured). On success the lead is stored in mmd_coursedev_lead and a
 * notification is emailed to the sales mailbox.
 */
class MMD_Coursedev_IndexController extends Mage_Core_Controller_Front_Action
{
    const RETURN_URL = 'course-development-service.html';
    ...
    public function postAction()
    {
        ...
        try {
            ...
            $lead = Mage::getModel('mmd_coursedev/lead')
                ...
                ->setMessage($comment)
                ->setSource((string) ($post['source'] ?? 'course-development-service'))
    ...
    /** Email the sales mailbox via the branded lead-notification template. */
    protected function _notify(MMD_Coursedev_Model_Lead $lead)
    {
        Mage::helper('mmd_leadmail')->notify('Course Development Enquiry', $lead->getName(), $lead->getEmail(), $lead->getTelephone(), array(
            array('Area of Expertise / Course Topic', $lead->getExpertise()),
            array('Company / Organisation', $lead->getCompany()),
        ), $lead->getMessage());
    }
}
