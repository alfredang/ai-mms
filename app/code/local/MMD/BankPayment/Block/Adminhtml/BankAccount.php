...
class MMD_BankPayment_Block_Adminhtml_BankAccount extends Mage_Adminhtml_Block_System_Config_Form_Field
{
    ...
    {
        $html = '<li><fieldset>';
        $html .= '<label style="display:block;margin:1px 0;">'.$this->__('Account Holder').'</label>';
        ...
        $html .= '<label style="display:block;margin:1px 0;">'.$this->__('Account Number').'</label>';
        ...
        $html .= '<label style="display:block;margin:1px 0;">'.$this->__('Bank Name').'</label>';
        $html .= '<input class="input-text" type="text" name="'.$this->getElement()->getName().'[bank_name][]" value="' . $this->_getValue('bank_name/'.$i) . '" '.$this->_getDisabled().' />';	
		 $html .= '<label style="display:block;margin:1px 0;">'.$this->__('Bank Code').'</label>';
        ...
        $html .= '<label style="display:block;margin:1px 0;">'.$this->__('Branch Code').'</label>';
        $html .= '<input class="input-text" type="text" name="'.$this->getElement()->getName().'[branch_code][]" value="' . $this->_getValue('branch_code/'.$i) . '" '.$this->_getDisabled().' />';
		
        $html .= '<label style="display:block;margin:1px 0;">'.$this->__('IBAN').'</label>';
        $html .= '<input class="input-text" type="text" name="'.$this->getElement()->getName().'[iban][]" value="' . $this->_getValue('iban/'.$i) . '" '.$this->_getDisabled().' />';
        $html .= '<label style="display:block;margin:1px 0;">'.$this->__('BIC').'</label>';
