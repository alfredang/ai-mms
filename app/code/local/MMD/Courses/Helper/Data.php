...

{
    ...
    /** Current like count for a product (0 if no row yet). */
    public function getProductLikes($productId)
    {
        $resource = Mage::getSingleton('core/resource');
        $read     = $resource->getConnection('core_read');
        return (int) $read->fetchOne(
            $read->select()
                ->from($resource->getTableName('mmd_product_like'), 'likes')
     ...
     * Mirrors MMD_Blog_Helper_Data::likePost.
     *
    ...
    public function likeProduct($productId)
    {
        ...
        $voteTable = $resource->getTableName('mmd_product_like_vote');
        $likeTable = $resource->getTableName('mmd_product_like');

        $hash = sha1($productId . '|' . Mage::helper('core/http')->getRemoteAddr() . '|'
            . Mage::helper('core/http')->getHttpUserAgent());

        // INSERT IGNORE returns affected rows = 1 for a new like, 0 for a repeat.
        $stmt = $write->query(
            "INSERT IGNORE INTO {$voteTable} (product_id, voter_hash, created_at)
             VALUES (:product_id, :hash, NOW())",
            array('product_id' => $productId, 'hash' => $hash)
        );
        $isNew = ((int) $stmt->rowCount()) > 0;
        if ($isNew) {
            $write->query(
                "INSERT INTO {$likeTable} (product_id, likes) VALUES (:product_id, 1)
        ...
        $likes = (int) $write->fetchOne(
            $write->select()->from($likeTable, 'likes')->where('product_id = ?', $productId)
        );
