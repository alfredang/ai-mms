 ...
 */
class MMD_Courses_Block_Adminhtml_Providers_Grid_Renderer_Provider extends Mage_Adminhtml_Block_Widget_Grid_Column_Renderer_Abstract
{
    ...
    public function render(Varien_Object $row)
	{
		...
		$code = Mage::getStoreConfig('courses/general/providers_code');				
		  ...
		  if($code){			
			  foreach ( $providers->getSource()->getAllOptions(true, true) as $option){
					if($option['value']!='' && $option['value']==$id){
						$result =  Mage::helper('courses')->__($option['label']);
					}
			  }
		  }
		return $result;
