...
var mmdCatalogmanagementSortable = new Class.create();

mmdCatalogmanagementSortable.prototype = {
    ...
    init: function()
    {
        ...
        if (this.canSort)
        {
            $(this.sortButtonId).childElements()[0].childElements()[0].childElements()[0].innerHTML = this.textSave;
            $$('#' + this.tableId + ' .headings th:not([class~=no-link])').each(function(th){
                th.addClassName('catalogmanagement_sortable');
            });
        ...
        } else
        {
            ...
            $$('#' + this.tableId + ' .headings th:not([class~=no-link]) a').each(function(a){
                postData += '&fields[]=' + a.name;
            });
            // save columns order
            new Ajax.Request(this.saveSortingUrl, {
                ...
                onSuccess: function(transport) {
                    $(this.sortButtonId).childElements()[0].childElements()[0].childElements()[0].innerHTML = this.textSort;
                    $$('#' + this.tableId + ' .headings th:not([class~=no-link])').each(function(th){
                        th.removeClassName('catalogmanagement_sortable');
                    });
...
dragtable = {
    ...
    dragMove: function(event) {
        ...
        $$('.headings th').each(function(th, i) {
            if (i == targetCol)
            {
                th.addClassName('catalogmanagement_sortable_insertafter');
            } else
            {
                th.removeClassName('catalogmanagement_sortable_insertafter');
            }
        });
