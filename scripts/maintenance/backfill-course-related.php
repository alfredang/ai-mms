<?php
/**
 * One-shot backfill: guarantee every SG WSQ course (SKU "TGS-") has at least
 * MIN_RELATED (5) *valid* Related links — the storefront "Related Courses" tab
 * (sibling of the "Recommended Courses" upsell tab on the product page).
 * Courses that fall short are topped up to 5 with courses from the same
 ...
 *
 * Candidates are SG courses (TGS- and C-prefix). A candidate already linked as
 * an UPSELL of the same product is skipped, so the two tabs of the Recommended
 * block never show the same tile twice.
 *
 * "Valid" = the linked product is enabled + catalog/search-visible, because the
 * frontend related block only renders those; links to disabled/deleted products
 * don't count toward the 5.
 *
 * Existing related links are never removed or reordered — new links are
 * appended after them (position). Idempotent: candidates already linked are
 * skipped; the unique key (link_type_id, product_id, linked_product_id) +
 * INSERT IGNORE make re-runs safe.
 *
 * On a partner DB (MY/GH, no TGS- courses) the target set is empty and the
 * script is a pure no-op — but this is a one-shot maintenance script, NOT a
 * repo migration, and is meant to be run on the SG server only.
 *
 * Usage (inside the web container on the SG server):
 *   php scripts/maintenance/backfill-course-related.php
 *       -> DRY RUN (default): reports scope + exactly what it *would* add. No writes.
 *   php scripts/maintenance/backfill-course-related.php --commit
 *       -> writes the related links, then flushes block_html + FPC caches.
 *   php scripts/maintenance/backfill-course-related.php --commit --no-flush
...
$MIN_RELATED      = 5;
$RELATED_TYPE_ID  = 1;   // catalog_product_link_type.code = relation
$UPSELL_TYPE_ID   = 4;   // used only to exclude cross-tab duplicates
$POSITION_ATTR_ID = 1;   // catalog_product_link_attribute (link_type 1, code 'position')
...
echo "== backfill-course-related ==\n";
echo "mode: " . ($commit ? "COMMIT" : "DRY RUN (default)") . "\n";
echo "min related: {$MIN_RELATED}   store root cat: {$STORE_ROOT_CAT}\n\n";
 ...
 *      $catCourse[catId] = [coursePid,...] visible SG courses in that category
 * ------------------------------------------------------------------------- */
$prodCats  = array();
$catCourse = array();
...
foreach ($read->fetchAll($sql) as $r) {
    ...
    $prodCats[$pid][] = $cid;
    if (isset($sgCourse[$pid])) { $catCourse[$cid][] = $pid; }
}
...
/* ---------------------------------------------------------------------------
 * 5. Existing links.  $rel[pid] = [linkedId => true] ; $ups[pid] = same (for
 *    cross-tab dedup only).
 * ------------------------------------------------------------------------- */
$rel = array();
$ups = array();
$sql = "SELECT product_id, linked_product_id, link_type_id FROM {$tLink}
        WHERE link_type_id IN ({$RELATED_TYPE_ID},{$UPSELL_TYPE_ID})";
foreach ($read->fetchAll($sql) as $r) {
    $pid = (int) $r['product_id'];
    $lid = (int) $r['linked_product_id'];
    if ((int) $r['link_type_id'] === $RELATED_TYPE_ID) { $rel[$pid][$lid] = true; }
    else { $ups[$pid][$lid] = true; }
}

/* ---------------------------------------------------------------------------
 * 6. Walk WSQ courses; compute shortfall; pick related courses; collect inserts.
 * ------------------------------------------------------------------------- */
...
foreach (array_keys($isWsq) as $pid) {
    $existing  = isset($rel[$pid]) ? $rel[$pid] : array();
    $upsellSet = isset($ups[$pid]) ? $ups[$pid] : array();
    // valid = existing link whose target is currently visible
    $validCount = 0;
    foreach ($existing as $lid => $_) {
        if (isset($visible[$lid])) { $validCount++; }
    }
    $need = $MIN_RELATED - $validCount;
    if ($need <= 0) { continue; }
    $deficient++;
    ...
    $score = function (array $catSet) use ($pid, $existing, $upsellSet, $catCourse) {
        $out = array();
        foreach ($catSet as $cid) {
            if (empty($catCourse[$cid])) { continue; }
            foreach ($catCourse[$cid] as $cand) {
                if ($cand === $pid) { continue; }
                if (isset($existing[$cand])) { continue; }
                if (isset($upsellSet[$cand])) { continue; } // already on the Recommended tab
    ...
    if (empty($poolScore)) {
        $stillShort[$pid] = array($validCount, $MIN_RELATED, 0);
        continue;
    }
    ...
    if (count($pick) < $need) {
        $stillShort[$pid] = array($validCount, $MIN_RELATED, $validCount + count($pick));
    }
...
echo "WSQ courses short of {$MIN_RELATED}: {$deficient}\n";
echo "related links to add: {$addedTotal}\n";
echo "courses still < {$MIN_RELATED} after fill (thin categories): " . count($stillShort) . "\n\n";
 ...
 * ------------------------------------------------------------------------- */
echo "\nWriting " . count($plan) . " related links...\n";
...
try {
    ...
    foreach ($plan as $row) {
        ...
        $write->query(
            ...
            array($pid, $cand, $RELATED_TYPE_ID)
        );
