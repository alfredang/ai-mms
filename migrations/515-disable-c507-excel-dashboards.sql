...
--   C507 - Creating an Interactive Excel Dashboards for Business Analytics
--
-- Retired because C34 "Microsoft 365 Copilot for Excel" already covers the
-- modern Copilot-driven Excel analytics offering.
...
-- Flip any per-store status override rows to Disabled as well (an Enabled
-- override keeps the course live on that store even when store_id 0 is
-- Disabled).
UPDATE catalog_product_entity_int i
JOIN catalog_product_entity e ON e.entity_id = i.entity_id
SET i.value = 2
WHERE i.attribute_id = @status_attr AND e.sku IN ('C507');
