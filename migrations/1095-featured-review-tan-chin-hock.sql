-- 1095: Featured review — Tan Chin Hock (WhatsApp, 2026-08-23).
--
-- Seeds the 5-star testimonial Tan Chin Hock (ACLP trainer) sent by WhatsApp
-- on 2026-08-23 thanking Dr Alfred for the Claude workshop, as a normal
-- approved product review flagged `is_featured = 1` (same pattern as
-- migrations 897/911/920/928), so it shows on the homepage "What our learners
-- say" strip, /testimonials, and the course's own review list.
--
-- Course: TGS-2025052468 "WSQ - Agentic AI Applications with Claude Code" —
-- the learner's own enrolment (order 100041147, class 2026-05-28,
-- chinhock@holdinghands.sg) confirms which Claude workshop this refers to.
--
-- NOTE: this learner ALREADY has an approved non-featured review on the same
-- course (review_id 22620, 2026-05-30, "Very fruitful learning..."). This is a
-- SECOND, later testimonial, so the NOT EXISTS guard is keyed on the new
-- review's title as well as the nickname — matching on nickname alone would
-- see 22620 and skip the seed forever.
--
...
-- the whole file no-ops there. `@rid` is derived from ROW_COUNT() so a skipped
...
WHERE @sg = 1
  ...
  AND NOT EXISTS (
      ...
      WHERE d.nickname = 'Tan Chin Hock'
        AND d.title = 'Structured so easily to follow'
...
SELECT @rid, 1,
       'Structured so easily to follow',
       'My biggest THANK YOU for the workshop on Claude! Cos you structured it so easily to follow, and easy to execute, I digested well. Thank you 师父 (master)!!',
       'Tan Chin Hock', NULL
...
FROM review r
JOIN review_detail d ON d.review_id = r.review_id
JOIN catalog_product_entity e ON e.entity_id = r.entity_pk_value AND e.sku = 'TGS-2025052468'
...
SET @pid := (SELECT entity_id FROM catalog_product_entity WHERE sku = 'TGS-2025052468');
