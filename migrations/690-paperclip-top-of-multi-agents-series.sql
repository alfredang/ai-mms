-- 690: Pin "WSQ - Manage AI Agents with Paperclip" (TGS-2023036646) to the TOP
-- of Multi Agents Series. WSQ rows keep relative order through the canonical
-- reorder, so position 0 + the 691 reorder makes it #1 durably.
...
SET @cat := (SELECT entity_id FROM catalog_category_entity_varchar
  WHERE attribute_id = (SELECT attribute_id FROM eav_attribute WHERE entity_type_id = 3 AND attribute_code = 'url_key')
    AND store_id = 0 AND value = 'multi-agents-series' LIMIT 1);
SET @p := (SELECT entity_id FROM catalog_product_entity WHERE sku = 'TGS-2023036646');
