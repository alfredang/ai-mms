-- 793: Add the Skills Framework section (TSC Title + TSC Code) to
-- TGS-2020505042 "WSQ - AI Vibe Coding with React", whose short_description
-- had no Skills Framework block, so the storefront "Skills Framework" card
-- (TSC Title / TSC Code) never rendered.
--
--   TSC Title : Applications Development-3
--   TSC Code  : ICT-DIT-3002-1.1
--
-- Note: the Vibe Coding siblings do NOT all share one TSC. So far:
...
--   793     Applications Development-3  ICT-DIT-3002-1.1 (React, this file)
-- Each course's TSC is supplied per-course; never inferred from a sibling.
--
-- Same shape as migrations 789-792: the product-view template derives the
...
SET @e := (SELECT entity_id FROM catalog_product_entity WHERE sku = 'TGS-2020505042');

UPDATE catalog_product_entity_text
SET value = CONCAT(value, '\n', '<h2>Skills Framework</h2>
<p>This course follows the guideline of&nbsp;<strong>Applications Development-3 ICT-DIT-3002-1.1 TSC</strong> under ICT Skills Framework</p>')
WHERE entity_id = @e
  AND @e IS NOT NULL
