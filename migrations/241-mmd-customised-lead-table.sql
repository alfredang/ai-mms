-- 241: Customised Training enquiry store for MMD_Customised.
CREATE TABLE IF NOT EXISTS `mmd_customised_lead` (
    ...
    `company` VARCHAR(255) NOT NULL DEFAULT '', `num_pax` VARCHAR(32) NOT NULL DEFAULT '',
    `training_topic` VARCHAR(255) NOT NULL DEFAULT '', `preferred_dates` VARCHAR(128) NOT NULL DEFAULT '',
    ...
    PRIMARY KEY (`lead_id`), KEY `IDX_CUST_LEAD_CREATED` (`created_at`), KEY `IDX_CUST_LEAD_STATUS` (`status`)
