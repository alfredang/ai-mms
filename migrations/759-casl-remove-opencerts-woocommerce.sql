-- 759: Re-run the CASL OpenCerts rule (see 747/748) for the newly converted
-- CASL - Building a Successful eCommerce Store with WooCommerce
-- (TGS-2026064474). Earlier copies already ran on prod before this
...
-- and LF line endings. The storefront template rebuilds the Certification
-- card and already suppresses OpenCerts for CASL names, so this is the
-- belt-and-braces data cleanup. Idempotent; partner-safe.
