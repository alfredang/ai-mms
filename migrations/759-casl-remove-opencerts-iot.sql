-- 759: Re-run the CASL OpenCerts rule (see 747/748/751) for the newly
-- converted CASL - Business Innovation with Internet-of-Things (IoT)
-- (TGS-2026064711). The earlier name-driven cleanups run BEFORE 757 renames
-- this course to CASL, so the rule must ship again as a NEW file (edited
-- migrations never re-run). Removes the OpenCerts bullet from the
-- Certification section of every course named 'CASL - %', matching BOTH CRLF
-- and LF line endings. The storefront template already suppresses OpenCerts
-- for CASL names; this is the belt-and-braces data cleanup.
-- Idempotent; partner-safe.
