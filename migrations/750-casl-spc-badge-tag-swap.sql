-- 750: CASL funding badge for TGS-2026064862 (CASL - Statistical Process
-- Control (SPC) in Manufacturing): swap this product's WSQ badge tag for the
-- canonical CASL tag (seeded by 746; re-seeded here guarded for safety).
-- Storefront chips + cover (set in 749) now both say CASL.
-- Partner-safe: TGS- SKU absent on MY/GH => @e NULL => guarded no-op.

SET @e := (SELECT entity_id FROM catalog_product_entity WHERE sku = 'TGS-2026064862');
