-- Refresh course covers for C148 (Google Workspace Masterclass) and C23
-- (Adobe InDesign CC Masterclass). Their course_image_url still pointed at the
-- 2026-05-23 covers rendered with the OLD pre-repurpose titles, so the product
-- image did not match the course title. New covers rendered 2026-07-17 with
-- the current names. Also aligns the image label attributes.
...
SELECT 4, @a_ciu, 0, @e, 'https://pub-77c0dec029944b0386e40673ce81081f.r2.dev/course-covers/C148-20260717-161309.png' FROM DUAL WHERE @e IS NOT NULL AND @a_ciu IS NOT NULL
...
INSERT INTO catalog_product_entity_varchar (entity_type_id, attribute_id, store_id, entity_id, value)
SELECT 4, @a_ciu, 0, @e, 'https://pub-77c0dec029944b0386e40673ce81081f.r2.dev/course-covers/C23-20260717-161310.png' FROM DUAL WHERE @e IS NOT NULL AND @a_ciu IS NOT NULL
ON DUPLICATE KEY UPDATE value = VALUES(value);
INSERT INTO catalog_product_entity_varchar (entity_type_id, attribute_id, store_id, entity_id, value)
