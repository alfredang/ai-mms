-- Re-apply the curated non-WSQ category orders AGAIN (second time — first
-- was 650): a parallel session's 651/652 rollout (enable-all-WSQ / disable
-- Dynamics-365) re-ran the canonical global reorder on prod after 650,
-- flattening the Claude AI Series / Python / React curated orders back to
-- alphabetical. Verbatim re-issue of the 642/645/648 position writes.
-- Rule now recorded in .claude/skills/category-ordering/SKILL.md: every
-- canonical global reorder MUST be followed by a re-apply of the curated
-- category files in the same push.
--
