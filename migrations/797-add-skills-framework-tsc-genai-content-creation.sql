-- 797: Add the Skills Framework section (TSC Title + TSC Code) to
-- TGS-2023037589 "WSQ - Generative AI for Content Creation", whose
-- short_description had no Skills Framework block, so the storefront
-- "Skills Framework" card (TSC Title / TSC Code) never rendered.
--
--   TSC Title : Content Strategy-5
--   TSC Code  : ICT-SNM-5004-1.1
--
-- Level-5 sibling of migration 795 (Power Automate + Copilot Studio,
-- Content Strategy-4 / ICT-SNM-4004-1.1). Each course's TSC is supplied
-- per-course; never inferred from a sibling.
--
-- Same shape as migrations 789-796: the product-view template derives the
...
SET @e := (SELECT entity_id FROM catalog_product_entity WHERE sku = 'TGS-2023037589');

UPDATE catalog_product_entity_text
SET value = CONCAT(value, '\n', '<h2>Skills Framework</h2>
<p>This course follows the guideline of&nbsp;<strong>Content Strategy-5 ICT-SNM-5004-1.1 TSC</strong> under ICT Skills Framework</p>')
WHERE entity_id = @e
  AND @e IS NOT NULL
