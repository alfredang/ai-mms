-- Re-apply the curated non-WSQ category orders that migration 613 (global
-- canonical reorder, 545 pattern) flattened back to alphabetical.
--
-- Why this happened: 613 was authored in an earlier session but only reached
-- production in the 2026-07-21 backlog push — so on prod it executed AFTER
-- the curated-order migrations 642 (Python), 645 (React) and 648 (Claude AI
-- Series), clobbering all three (ledger order != filename order). This file
-- re-issues their position writes verbatim. Each curated file already warns:
-- "re-apply this file after any such reorder".
--
-- Positions 101+ keep each curated block safely after the WSQ rows.
-- Categories resolved by url_key, products by SKU: no-ops where absent.
-- Idempotent.
