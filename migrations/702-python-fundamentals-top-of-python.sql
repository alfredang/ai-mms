-- 702: Pin "WSQ - Python Fundamental Course for Beginners" (TGS-2019503161)
-- to the TOP of the Python category. WSQ rows keep relative order through the
-- canonical reorder, so position 0 + the 703 reorder makes it #1 durably.
...
SET @cat := (SELECT entity_id FROM catalog_category_entity_varchar
  WHERE attribute_id = (SELECT attribute_id FROM eav_attribute WHERE entity_type_id = 3 AND attribute_code = 'url_key')
    AND store_id = 0 AND value = 'python-programming' LIMIT 1);
SET @p := (SELECT entity_id FROM catalog_product_entity WHERE sku = 'TGS-2019503161');
