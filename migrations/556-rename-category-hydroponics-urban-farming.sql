-- Rename the category "Hydroponics Urban Farming" -> "Urban Farming"
-- (category 20, level 4, under Adult Courses > Sustainability & ESG).
--
-- Name only. url_key stays 'hydroponics-urban-farming-courses' so the category
-- URL, existing links and SEO are unchanged (no rewrite churn, no 301 needed).
--
...
SET value = 'Urban Farming'
WHERE attribute_id = @a_name AND value = 'Hydroponics Urban Farming';
...
SET @s := IF((SELECT COUNT(*) FROM information_schema.TABLES WHERE TABLE_SCHEMA=DATABASE() AND TABLE_NAME='catalog_category_flat_store_1')>0,
  "UPDATE catalog_category_flat_store_1 SET name='Urban Farming' WHERE name='Hydroponics Urban Farming'", 'DO 0');
...
SET @s := IF((SELECT COUNT(*) FROM information_schema.TABLES WHERE TABLE_SCHEMA=DATABASE() AND TABLE_NAME='catalog_category_flat_store_2')>0,
  "UPDATE catalog_category_flat_store_2 SET name='Urban Farming' WHERE name='Hydroponics Urban Farming'", 'DO 0');
PREPARE st FROM @s; EXECUTE st; DEALLOCATE PREPARE st;
...
SET @s := IF((SELECT COUNT(*) FROM information_schema.TABLES WHERE TABLE_SCHEMA=DATABASE() AND TABLE_NAME='catalog_category_flat_store_3')>0,
  "UPDATE catalog_category_flat_store_3 SET name='Urban Farming' WHERE name='Hydroponics Urban Farming'", 'DO 0');
PREPARE st FROM @s; EXECUTE st; DEALLOCATE PREPARE st;
