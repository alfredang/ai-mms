-- 767: CASL funding badge for TGS-2026064472 (CASL - Hands-On Web App
-- Development with Javascript): swap this product's WSQ badge tag for the
-- canonical CASL tag (seeded guarded for safety). Storefront chips + cover
-- (set in 766) now both say CASL. Other badges (SkillsFuture Credit, PSEA,
-- SFEC, Absentee Payroll, MCES) are retained.
-- Partner-safe: TGS- SKU absent on MY/GH => @e NULL => guarded no-op.

SET @e := (SELECT entity_id FROM catalog_product_entity WHERE sku = 'TGS-2026064472');
