-- 298: DB performance quick wins (from mysql-memory-optimizer audit 2026-07-02)
...
SET @sql := IF(@idx > 0, 'ALTER TABLE course_runs DROP INDEX idx_product_date_time', 'DO 0');
PREPARE s FROM @sql;
EXECUTE s;
DEALLOCATE PREPARE s;
...
SET @sql := IF(@idx > 0, 'ALTER TABLE course_runs DROP INDEX idx_product', 'DO 0');
PREPARE s FROM @sql;
EXECUTE s;
DEALLOCATE PREPARE s;
...
SET @sql := IF(@idx > 0, 'ALTER TABLE course_run_enrolments DROP INDEX idx_product', 'DO 0');
PREPARE s FROM @sql;
EXECUTE s;
DEALLOCATE PREPARE s;
