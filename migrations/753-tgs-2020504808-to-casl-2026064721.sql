...
--   "WSQ - Robotics Process Automation (RPA) for Beginners" (TGS-2020504808)
--   -> "CASL - Robotics Process Automation (RPA) for Beginners" (TGS-2026064721)
...
SET @e := (SELECT entity_id FROM catalog_product_entity WHERE sku = 'TGS-2020504808');
...
UPDATE catalog_product_entity SET sku = 'TGS-2026064721'
  WHERE entity_id = @e
    AND NOT EXISTS (SELECT 1 FROM (SELECT sku FROM catalog_product_entity WHERE sku = 'TGS-2026064721') x);

-- Name + labels + fresh cover (rendered with the CASL chip replacing WSQ)
UPDATE catalog_product_entity_varchar SET value = 'CASL - Robotics Process Automation (RPA) for Beginners'
  WHERE entity_id = @e AND attribute_id IN (@a_name, @a_il, @a_sil, @a_til) AND store_id = 0;
UPDATE catalog_product_entity_varchar SET value = 'https://pub-77c0dec029944b0386e40673ce81081f.r2.dev/course-covers/TGS-2026064721-20260722-181450.png'
  WHERE entity_id = @e AND attribute_id = @a_ciu AND store_id = 0;

-- Media-gallery per-image label
UPDATE catalog_product_entity_media_gallery_value gv
  JOIN catalog_product_entity_media_gallery g ON g.value_id = gv.value_id
  SET gv.label = 'CASL - Robotics Process Automation (RPA) for Beginners'
  WHERE g.entity_id = @e;
...
-- SEO meta
UPDATE catalog_product_entity_varchar SET value = 'CASL Robotics Process Automation (RPA) with UIPath for Beginners - Automate Your Workflow | Tertiary Courses Singapore'
...
UPDATE catalog_product_entity_text SET value = REPLACE(value, 'TGS-2020504808', 'TGS-2026064721')
  WHERE entity_id = @e AND attribute_id = @a_sdesc;
UPDATE catalog_product_entity_text SET value = REPLACE(value, 'WSQ - Robotics Process Automation (RPA) for Beginners', 'CASL - Robotics Process Automation (RPA) for Beginners')
  WHERE entity_id = @e AND attribute_id = @a_sdesc;
UPDATE catalog_product_entity_text SET value = REPLACE(value, 'WSQ Robotics Process Automation (RPA) course', 'CASL Robotics Process Automation (RPA) course')
  WHERE entity_id = @e AND attribute_id = @a_sdesc;

-- Per-SKU cms_blocks: rename identifiers + repoint content to the new SKU
UPDATE cms_block SET identifier = 'course_TGS-2026064721_brochure',
    title = REPLACE(title, 'TGS-2020504808', 'TGS-2026064721'),
    content = REPLACE(content, 'TGS-2020504808', 'TGS-2026064721')
  WHERE identifier = 'course_TGS-2020504808_brochure';
UPDATE cms_block SET identifier = 'course_TGS-2026064721_learning_outcomes',
    title = REPLACE(title, 'TGS-2020504808', 'TGS-2026064721'),
    content = REPLACE(content, 'TGS-2020504808', 'TGS-2026064721')
  WHERE identifier = 'course_TGS-2020504808_learning_outcomes';
