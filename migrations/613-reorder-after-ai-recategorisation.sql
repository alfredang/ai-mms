-- 613-reorder-after-ai-recategorisation.sql
-- Verbatim copy of canonical 545 (see .claude/skills/category-ordering).
-- Ordering is data-derived, so it MUST be re-applied after migration 612
-- changes category membership: WSQ (TGS-) first, then non-WSQ (C-)
-- alphabetical by store-0 name, then M/other.
-- Shipped as a NEW file because an already-applied migration never re-runs
-- on prod (schema_migrations tracks by filename).
