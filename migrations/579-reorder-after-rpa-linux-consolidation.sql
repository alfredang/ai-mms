-- Re-apply the canonical WSQ-first / non-WSQ-alphabetical category ordering.
--
-- Required because 565/566/567 backfilled products into the RPA, Linux and
-- Web Development parents at position 0, which would otherwise leave them in
-- arbitrary order at the top of those listings.
--
-- This is a VERBATIM copy of the two UPDATE statements in
-- 545-category-ordering-index-selfcontained.sql, per the category-ordering
-- skill: the ledger tracks by filename, so an already-applied file never
-- re-runs on prod — a fresh numbered copy is the correct way to re-apply.
-- Do NOT copy 539/542 (they carry the anchor-inheritance INNER-JOIN bug).
--
-- Renumbers catalog_category_product_index DIRECTLY (self-contained, so
-- anchor-inherited rows are covered) per (category_id, store_id), then the
-- base table for the admin view. Idempotent and partner-safe (prefix
-- convention only, no SKU list).
-- After deploy: flush Redis / run the reindex API so flat + block/FPC pick up
-- the new positions.
