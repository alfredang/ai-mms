-- 754: CASL funding badge for TGS-2026064533 (CASL - Cyber Security Awareness
-- Course for Personal and Businesses): swap this product's WSQ badge tag for
-- the canonical CASL tag (seeded by 746; re-seeded here guarded for safety).
-- Storefront chips + cover (set in 753) now both say CASL. Other badges
-- (SkillsFuture Credit, PSEA, SFEC, Absentee Payroll, MCES) are retained.
-- Partner-safe: TGS- SKU absent on MY/GH => @e NULL => guarded no-op.

SET @e := (SELECT entity_id FROM catalog_product_entity WHERE sku = 'TGS-2026064533');
