-- 1248: Final non-WSQ order for the Claude AI Series, per the confirmed
-- nine-course list:
...
--   5. C141  Claude Code for Digital Marketing
...
--
-- Supersedes 1247, which had C141 parked at the end (it was absent from that
-- request's list, so it was pinned last rather than dropped); the follow-up
-- confirmed it belongs at position 5, between the Masterclasses and the
-- Certified courses.
--
-- Every non-WSQ member of the category is covered by the CASE, so none drifts
-- above the block. Positions 101..109 keep them after the three WSQ courses,
-- and the category is already in mmd/category_ordering/curated_url_keys so
-- the nightly sweep preserves this.
--
