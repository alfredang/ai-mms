-- 924: Remove the OpenCerts / SOA bullet from CASL certification blocks.
-- Same shape as 906 — re-shipped as a NEW file because 906 already ran on
-- prod and an applied migration never re-runs, so the newly converted
-- TGS-2026064861 (CASL - Data Analytics with Excel) would keep its bullet.
-- The Certification card renders from the per-course cms_block
