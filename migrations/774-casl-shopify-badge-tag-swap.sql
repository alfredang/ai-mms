-- 774: CASL funding badge for TGS-2026064175 (CASL - Running a Successful
-- eCommerce Store with Shopify): swap this product's WSQ badge tag for the
-- canonical CASL tag (seeded by 746; re-seeded here guarded for safety).
-- Storefront chips + cover (set in 773) now both say CASL.
-- Partner-safe: TGS- SKU absent on MY/GH => @e NULL => guarded no-op.

SET @e := (SELECT entity_id FROM catalog_product_entity WHERE sku = 'TGS-2026064175');
