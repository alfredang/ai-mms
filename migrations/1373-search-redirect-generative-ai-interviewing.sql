-- Search-term redirect: "Generative AI for Interviewing" -> the matching WSQ course page.
--
-- Keep this exact-match only so related interviewing searches continue to use
-- their own results or redirects. Create the search term when it does not exist
-- yet, then correct every exact duplicate deterministically.

SET @sg   := (SELECT COUNT(*) FROM core_store WHERE store_id = 1 AND code = 'singapore');
SET @term := 'Generative AI for Interviewing';
