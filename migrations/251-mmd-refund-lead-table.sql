-- 251: Refund Request store for MMD_Refund.
CREATE TABLE IF NOT EXISTS `mmd_refund_lead` (
  ...
  `course` VARCHAR(255) NOT NULL DEFAULT '', `order_ref` VARCHAR(128) NOT NULL DEFAULT '',
  ...
  PRIMARY KEY (`lead_id`), KEY `IDX_REFUND_CREATED` (`created_at`), KEY `IDX_REFUND_STATUS` (`status`)
